RSpec.describe 'GET /api/articles', type: :request do
  ...
  let!(:test_article2) { create(:article, created_at: Time.current - 4.days) }
  let!(:test_article3) { create(:article, created_at: Time.current - 3.days) }
  let!(:test_article4) { create(:article, title: 'Test Article 4', created_at: Time.current - 2.days) }
  ...
  describe 'successfully' do
    ...
    it 'responds with newest article first in the list' do
      ...
      expect(response_json['articles'].second['title']).to eq 'Test Article 4'
    end
  end
end
